...
class TestTheReportedTier:
    ...
    def test_a_bracket_span_with_no_citation_key_never_reaches_the_count(
        ...
        # question is now settled one layer up: every bracket span in prose is
        # a citation group, enforced by parse, so a span with no key never
        # reaches a diagnostic at all.
        where = paper("prose-diagnostics")
        source = where / "MANUSCRIPT.working.md"
        source.write_text(
            source.read_text().replace(
