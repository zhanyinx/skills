...
Build a paper's document from its `skeleton.md` and run the mechanical gate over it. It injects every heading from the skeleton on every pass, strips the author-facing comment channel by syntax, marks every gap as a conspicuous token, and reports a per-check verdict table in which a check that never looked is a printed row rather than a silent pass. The answer to *"is this paper done"* is an exit code: `--circulate` always emits, `--submit` refuses while any gate bit is open, `--check` runs the gate alone. A fourth mode, `--scaffold`, writes rather than reads: it seeds a unit's source with every anchor in its subtree, in skeleton order, so a drafting session cannot type a misordered or missing one.
